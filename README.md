...
| 🤖 **멀티 LLM 지원** | `.env` 한 줄로 Claude CLI / Gemini CLI / OpenAI API 등 전환 |
...
# Claude CLI — Claude Pro 구독에 포함 (기본값)
LLM_MODEL=claude-cli

# Gemini CLI — 무료 티어 (Gemini CLI 설치 필요)
...
| **작동 방식** | pypdf로 텍스트 추출 → LLM에 정리 프롬프트 전달 |
...
FastAPI 서버 (localhost:8000)   ← systemd 사용자 서비스
        │
        ├── Edit Agent  ──── llm_provider (CLI 또는 API)
        ├── Wiki Agent  ──── llm_provider + DuckDuckGo (fallback)
        ├── Quiz Agent  ──── llm_provider
        ├── Back Agent  ──── llm_provider + AgentMEMO SQLite
        │
  ...
  llm_provider.py
  ├── CLI 방식: claude-cli (claude -p) / gemini-cli (gemini -p)
  └── API 방식: LiteLLM → Claude / OpenAI / Gemini / Ollama / 100+ 모델
...
# 2. .env 설정
cp .env.example .env
# → LLM_MODEL 선택 (기본값: claude-cli)
# → API 방식 사용 시 해당 API 키 입력
...
> **퀴즈 에이전트 대화형 기능**
> 답변 입력창에 원하는 내용을 자유롭게 요청할 수 있습니다.
> - `"서술형으로 바꿔줘"` — 객관식 대신 서술형 문제로 재출제
> - `"더 어렵게 내줘"` / `"쉽게 내줘"` — 난이도 조정
> - `"다른 문제 내줘"` — 새 문제 세트 생성
> - `"3번 문제 해설 더 자세히"` — 특정 문제 추가 설명
>
> 단, 개념 설명 요청은 🧠 위키 에이전트 탭에서 해주세요.
...
| **Quiz Agent** | 문제 출제, 채점, 대화형 요청 처리 | 퀴즈 탭 |
