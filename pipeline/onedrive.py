"""
OneDrive client — rclone 기반 (Azure 앱 등록 불필요)

최초 설정 (1회):
  python pipeline/onedrive.py --auth
  → 브라우저에서 Microsoft 계정 로그인만 하면 완료

이후 사용:
  python pipeline/onedrive.py --ls /강의자료
...
HASH_STORE_PATH = ROOT / "data" / "file_hashes.json"
...
def auth():
    """rclone config으로 OneDrive 원격 설정 (브라우저 로그인만 필요)."""
    ...
    print("\n" + "="*60)
    print("OneDrive 연동 설정")
    print("="*60)
    print("아래 단계를 따라 설정해주세요:\n")
    print("  1. 'n' 입력 (New remote)")
    print(f"  2. 이름: '{RCLONE_REMOTE}' 입력")
    print("  3. Storage type: 'onedrive' 선택 (번호 입력)")
    print("  4. client_id, client_secret: 공백으로 Enter (기본값 사용)")
    print("  5. 나머지는 기본값 Enter")
    print("  6. 브라우저에서 Microsoft 계정 로그인")
    print("  7. 마지막에 'y' 확인\n")
    print("="*60 + "\n")

    # rclone config 는 interactive 이므로 사용자가 직접 실행해야 함
...
def main():
    parser = argparse.ArgumentParser(description="OneDrive rclone 헬퍼")
    parser.add_argument("--auth",   action="store_true", help="OneDrive 연동 설정 안내")
    ...
    if args.whoami:
        if _is_remote_configured():
            ...
            print(f"❌ '{RCLONE_REMOTE}' 미설정. 먼저 실행: python pipeline/onedrive.py --auth")
        return

    if args.ls is not None:
        files = list_files(args.ls)
        print(f"\nOneDrive '{args.ls}' — {len(files)}개 파일:")
        for f in files:
